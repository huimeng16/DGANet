...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    if g > 0:
        #BCEcls, BCEobj = FocalLoss1(BCEcls, g), FocalLoss(BCEobj, g)
        BCEcls = FocalLoss1(BCEcls, g)
...
def plot_images(images, targets, paths=None, fname='images.jpg', names=None, max_size=640, max_subplots=16):
    ...
    #if fname=='gt':
    #    mosaic = cv2.resize(mosaic, (int(ns * w * 0.5), int(ns * h * 0.5)), interpolation=cv2.INTER_AREA)
    #    cv2.imwrite('res/'+label[:-4]+'_gt.jpg', cv2.cvtColor(mosaic, cv2.COLOR_BGR2RGB))
    #else:
    #    mosaic = cv2.resize(mosaic, (int(ns * w * 0.5), int(ns * h * 0.5)), interpolation=cv2.INTER_AREA)
    #    cv2.imwrite('res/'+label[:-4]+'_pred.jpg', cv2.cvtColor(mosaic, cv2.COLOR_BGR2RGB))
    return mosaic
