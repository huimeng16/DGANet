...
def test(cfg,
         ...
         imgsz=416,
         conf_thres=0.001,
         iou_thres=0.5,  # for nms,iou_thres=0.6
         save_json=False,
         single_cls=False,
         augment=False,
         model=None,
    ...
    for batch_i, (imgs, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            if is_training:  # if model has loss hyperparameters
                loss += compute_loss(train_out, targets, model)[1][:4]  # GIoU, obj, cls
        ...
        # Plot images
        # if batch_i < 1:
        #    f = os.path.abspath(res)+'/test_batch%g_gt.jpg' % batch_i  # filename
        #    plot_images(imgs, targets, paths=paths, names=names, fname=f)  # ground truth
        #    f = os.path.abspath(res)+'/test_batch%g_pred.jpg' % batch_i
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog='test0.py')
